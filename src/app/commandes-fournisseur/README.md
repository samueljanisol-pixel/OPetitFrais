...
- **Langue de l’image** : `preferred_locale` du vendeur (`fr` / `ar-MA`). **Fournisseur sans marchands** (Station) : pas de vendeur → l’export suit la **locale UI** (arabe si l’app est en `ar-MA` : en-têtes, noms produit, UdV, RTL).
