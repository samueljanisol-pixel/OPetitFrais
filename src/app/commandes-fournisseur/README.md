...
- Bouton **Envoyer WhatsApp** (si le vendeur a un **téléphone** renseigné dans Paramètres → Vendeurs) : image pré-capturée dans la **langue du vendeur** (`preferred_locale`). **Mobile** : partage natif du PNG (`navigator.share`) — choisir WhatsApp dans la feuille de partage, l’image est jointe. **Desktop** : copie de l’image dans le presse-papiers puis ouverture de la discussion `wa.me` (comme le panier boutique) — coller l’image avec Ctrl+V dans WhatsApp. Pas de texte dans l’URL (le commentaire est déjà dans l’image).
