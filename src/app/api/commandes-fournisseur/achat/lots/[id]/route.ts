...
async function applyLineUpdates(
  ...
  for (const u of updates) {
    ...
    const { data: ligne, error: le } = await supabase
      .from("commande_fournisseur_lot_ligne")
      .select(
        "id, lot_id, product_id, product_packaging_id, qte_achat, vendeur_id, marque_achete, prix_achat_unitaire, montant_ligne_achat, product(id, name, name_ar, ref_sales_unit(label), product_packaging(id, quantity, nom, ref_conditionnement(label), ref_sales_unit(label)))",
      )
      .eq("id", u.lotLigneId)
      .maybeSingle();
