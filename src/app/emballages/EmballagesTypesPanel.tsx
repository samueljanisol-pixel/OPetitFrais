...
export default function EmballagesTypesPanel({ canWrite, onError }: Props) {
  const t = useTranslations('backoffice.emballages')
  const tCommon = useTranslations('common')

  const [rows, setRows] = useState<EmballageTypeRow[]>([])
  const [loading, setLoading] = useState(false)
  const [open, setOpen] = useState(false)
  const [editing, setEditing] = useState<EmballageTypeRow | null>(null)
  const [label, setLabel] = useState('')
  const [active, setActive] = useState(true)
  const [saving, setSaving] = useState(false)
  const [reordering, setReordering] = useState(false)
  const [toDelete, setToDelete] = useState<EmballageTypeRow | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    onError(null)
    try {
      const res = await fetch('/api/emballages/types', { credentials: 'include' })
      const j = (await res.json().catch(() => ({}))) as { error?: string; types?: EmballageTypeRow[] }
  ...
  const openEdit = (row: EmballageTypeRow) => {
    setEditing(row)
    setLabel(row.label)
  ...
  const moveType = async (id: string, direction: -1 | 1) => {
    ...
    try {
      const results = await Promise.all(
        updates.map((u) =>
          fetch(`/api/emballages/types/${u.id}`, {
            method: 'PATCH',
            credentials: 'include',
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = editing
        ? await fetch(`/api/emballages/types/${editing.id}`, {
            method: 'PATCH',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
          })
        : await fetch('/api/emballages/types', {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
  ...
  const remove = async () => {
    ...
    onError(null)
    try {
      const res = await fetch(`/api/emballages/types/${toDelete.id}`, {
        method: 'DELETE',
        credentials: 'include',
      })
      const j = (await res.json().catch(() => ({}))) as { error?: string }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {canWrite ? (
          <Button variant="contained" color="success" onClick={openNew} sx={{ textTransform: 'none' }}>
            {tCommon('add')}
          </Button>
        ) : null}
      </div>
      ...
      {loading ? (
        ...
        <Typography className="!text-slate-600">{t('typesEmpty')}</Typography>
      ) : (
                      ...
                      {canWrite ? (
                        <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 0.25 }}>
                          <IconButton
                            ...
                            onClick={() => void moveType(row.id, -1)}
                          >
      ...
      <FormDialog
        ...
        maxWidth="sm"
      >
        <DialogTitle>{editing ? t('editType') : t('newType')}</DialogTitle>
        <DialogContent>
      ...
      <Dialog open={!!toDelete} onClose={() => (!saving ? setToDelete(null) : undefined)}>
        ...
        <DialogContent>
          <Typography>{t('deleteTypeBody', { label: toDelete?.label ?? '' })}</Typography>
        </DialogContent>
        <DialogActions>
