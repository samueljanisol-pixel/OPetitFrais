...
- **Dispatcher** : `GET/POST /api/automated-tasks/tick` — appelé par Vercel Cron toutes les **5 minutes** (`vercel.json`). Auth : `CRON_SECRET` ou `SYNC_TOKEN` (header `Authorization: Bearer`, `x-cron-secret`, ou `?token=`).
- **Registre** : tables `automated_tasks` / `automated_task_runs` (migration `20260722140000_automated_tasks.sql`).
- **UI** : Paramètres → onglet **Tâches automatisées** (administrateur) — activation, période, lancement manuel, historique.
- **Tâches** : `ftp_sync` (CA), `sheet_import` (Google Sheet → produits).
- **Compatibilité** : `POST /api/supabase/sync/run` (Raspberry Pi) délègue à la tâche `ftp_sync`. Statut footer : `/api/supabase/sync/status` lit `automated_task_runs` en priorité.
